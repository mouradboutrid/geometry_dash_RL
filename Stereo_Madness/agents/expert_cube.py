class CubeExpert:
    """
    Aligned reward shaping for Cube (Stereo Madness).
    ...
    def get_reward(state, action, prev_percent, prev_dist_nearest_hazard=None, reward_context=None):
        ...
        # Tuned parameters for convergence & exploration (Observing player behavior and adjusting ...)
        ...
        # Reduced jump penalty to allow necessary jumps (very low encouraging trying over precision)
        ...
        # Small per-step penalty (anti-idle)
        reward -= step_penalty
        ...
        # Landing on obstacles/blocks higher than player (strategic jumps)
        ...
        # Hazard cleaning reward (if within ±20 dy and agent passes/survives)
        if prev_dist_nearest_hazard is not None and hasattr(state, "dist_nearest_hazard"):
